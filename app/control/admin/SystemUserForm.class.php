...
class SystemUserForm extends TPage
{
    ...
    function __construct()
    {
		...
		$profissional_id     = new TDBCombo('profissional_id','database','ProfissionalRecord','id','nomeprofissional');
		...
		$profissional_change_action = new TAction(array($this, 'profissionalChangeAction')); 
		$profissional_id->setChangeAction($profissional_change_action); 
		...
		$this->form->addFields( [new TLabel('Profissional')], [$profissional_id] );
	...
	public static function profissionalChangeAction($param) {
		TTransaction::open('database');
		if (isset($param['profissional_id'])) {
			if ($param['profissional_id'] > 0) {
				$repository = new TRepository( "SystemUser" );

				$criteria = new TCriteria;
				$criteria->add(new TFilter("profissional_id", "=", $param["profissional_id"]));
				...
				$object = new ProfissionalRecord($param['profissional_id']);
				$data = new stdClass;
				
				if ($count == 0) {
					$login = str_replace('.', '', $object->numerocpf);
					$login = str_replace('-', '', $login);
					
					$data->name = $object->nomeprofissional;
					$data->login = $login;
					...
					$data->login = '';
					$data->email = '';
					new TMessage('error', 'Esse Profissional já foi vinculado a um usuário!');
				}
    ...
    public static function onSave($param)
    {
        try
        {
            ...
            if( $object->password )
            {
                ...
                $object->password = hash('sha256', $object->password . '@X3iYQHayDulkMl0@');
            }
            else
            {
			...
			unset($object->repassword);
			unset($object->program_id);
			unset($object->program_name);
